...
__global__ void kernel(uchar4* pos, unsigned int width, unsigned int height, float time)
{
    ...
    float freq = 4.0f;
    float w = abs(sinf(u*freq + time) * cosf(v*freq + time) * 0.5f);

    // write output vertex
    pos[y*width+x] = make_uchar4(u*255, w*255, v*255, 255);
}

// Wrapper for the __global__ call that sets up the kernel call
extern "C" void runKernel(uchar4* pos, unsigned int width, unsigned int height, float time)
{
    // execute the kernel
    dim3 block(8, 8, 1);
